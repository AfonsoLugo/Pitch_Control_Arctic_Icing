...
d_ice = file['d_tu_ice']
l_ice = file['l_tu_ice']
l_clean = file['l_tu_clean']
d_clean = file['d_tu_clean']
...
for i in range(len(angle)):
    ...
    cp_6 = (16/27)*lbda*( (Nb**(2/3) / (1.48 + (Nb**(2/3) - 0.04)*lbda + 0.0025*(lbda**2))) - ((d_ice[6]/l_ice[6])*(1.92*Nb*lbda/(1+2*Nb*lbda))))
...
## Maximum Cp and optimum lambda
for i in range(len(angle)):
    cp_c_6 = (16/27)*lbda*( (Nb**(2/3) / (1.48 + (Nb**(2/3) - 0.04)*lbda + 0.0025*(lbda**2))) - ((d_clean[6]/l_clean[6])*(1.92*Nb*lbda/(1+2*Nb*lbda))))

cp_max_c_6 = np.nanmax(cp_c_6) # Maximum Cp for angle = 0
print('The maximum Cp for clean condition is: '+str(round(cp_max_c_6,4)))
lbda_c_opt = lbda[np.nanargmax(cp_c_6)] # Lambda for maximum Cp
print('The optimum lambda for clean condition is: '+str(round(lbda_c_opt,1)))

# Cp for icing case using parameters of clean case
index_lbdaoptc = np.nanargmax(cp_c_6) #index of the optimum lambda for clean case
cp_optangle = cp_5[index_lbdaoptc] #cp of the icing for the optimum lambda for clean case
print('Cp with only optimized angle: ' + str(round(cp_optangle,4)))

# Cp with no optimization
cp_nonopt = cp_6[index_lbdaoptc]
...
plt.plot(lbda,cp_6, label="α = 6º", color = '#565656') # Curve for non-optimized angle
...
plt.annotate('Cp,opt = '+str(round(cp_max_5,4)), xy=(lbda_opt, cp_max_5), xytext=(lbda_opt-2.2, cp_max_5+0.022))
